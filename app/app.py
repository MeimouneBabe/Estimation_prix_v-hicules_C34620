...
from cardekho_styles import (
    ENHANCED_CSS, NAVBAR_HTML, FOOTER_HTML,
    HERO_HTML, DATASET_TABLE_HTML, result_card_html,
)
...
# ─────────────────────────────────────────────────────────────────
# NAVIGATION — le bouton hero met ?go=app dans l'URL
# ─────────────────────────────────────────────────────────────────
params = st.query_params
if "go" not in st.session_state:
    st.session_state.go = params.get("go", "home")

# Sync URL → session (clic bouton hero)
if params.get("go") == "app" and st.session_state.go == "home":
    st.session_state.go = "app"
...
# PAGE : ACCUEIL
# ══════════════════════════════════════════════════════════════════
if st.session_state.go == "home":

    # Hero complet avec bouton CTA + stats intégrés
    st.markdown(HERO_HTML, unsafe_allow_html=True)

    # ── Guide d'utilisation ───────────────────────────────────────
    st.markdown('<p class="section-label">Guide d\'utilisation</p>', unsafe_allow_html=True)
    ...
    st.markdown('<p class="section-label">À propos des données</p>', unsafe_allow_html=True)
    st.markdown("### Le jeu de données CarDekho")
...
with tab1:
    ...
    with algo_col:
        ...
        selected_model_name = st.radio(
            "Algorithme", list(MODELS.keys()), index=0,
        ...
        <div class="algo-metrics-grid">
            <div class="algo-metric"><div class="algo-metric-label">R² Test</div>
                <div class="algo-metric-value">{m['r2']:.3f}</div></div>
            <div class="algo-metric"><div class="algo-metric-label">CV R²</div>
                <div class="algo-metric-value">{m['cv_r2_mean']:.3f}</div></div>
            <div class="algo-metric"><div class="algo-metric-label">MAE</div>
                <div class="algo-metric-value">{m['mae']/100_000:.2f}L ₹</div></div>
            <div class="algo-metric"><div class="algo-metric-label">RMSE</div>
                <div class="algo-metric-value">{m['rmse']/100_000:.2f}L ₹</div></div>
        </div>""", unsafe_allow_html=True)
    ...
    with form_col:
        ...
        with fc1:
            brand = st.selectbox("Marque", SORTED_BRANDS,
                index=SORTED_BRANDS.index("Maruti") if "Maruti" in SORTED_BRANDS else 0)
        with fc2:
            year = st.number_input("Année de fabrication", min_value=1992, max_value=2024, value=2015, step=1)
        fc3, fc4 = st.columns(2)
        with fc3:
            km_driven = st.number_input("Kilomètres parcourus", min_value=0, max_value=500_000, value=50_000, step=1000)
        with fc4:
        ...
        with fc8:
            st.metric("Âge du véhicule", f"{REFERENCE_YEAR - year} ans")

        st.markdown("")
        predict_btn = st.button("Prédire le Prix", use_container_width=True, type="primary", key="predict_manual")

        if predict_btn:
            try:
                input_df  = preprocess_single(year, km_driven, FUEL_FR_TO_EN[fuel_fr],
                                              SELLER_FR_TO_EN[seller_type_fr], transmission, owner_label, brand)
                pred      = _predict(selected_model_name, selected_model, input_df)
                all_preds = {mn: _predict(mn, mo, input_df) for mn, mo in MODELS.items()}
                st.markdown(result_card_html(pred, selected_model_name, f"{brand} · {year} · {km_driven:,} km"),
    ...
    if predict_btn:
        try:
            st.markdown("---")
            st.markdown("#### Comparaison des prédictions — tous modèles")
            ...
            with st.expander("Données envoyées au modèle (après prétraitement)"):
                st.dataframe(input_df.T.rename(columns={0: "Valeur standardisée"}), use_container_width=True)
        except Exception:
            pass

# ── TAB 2 : IMPORT CSV / EXCEL ────────────────────────────────────
with tab2:
    _bkey = {"XGBoost (Meilleur)":"XGBoost","Random Forest":"Random Forest",
             "Régression Linéaire":"Régression Linéaire","SVR":"SVR"}
    ...
    with b_algo_col:
        ...
        selected_batch_model_name = st.radio(
            "Algorithme batch", list(MODELS.keys()), index=0,
            label_visibility="collapsed",
            format_func=lambda x: (
                f"XGBoost  ·  MEILLEUR\nR² = {_br2[x]:.4f}" if "XGBoost" in x
                else f"{x}\nR² = {_br2[x]:.4f}"
            ),
        ...
        <div class="algo-metrics-grid">
            <div class="algo-metric"><div class="algo-metric-label">R² Test</div>
                <div class="algo-metric-value">{bm['r2']:.3f}</div></div>
            <div class="algo-metric"><div class="algo-metric-label">CV R²</div>
                <div class="algo-metric-value">{bm['cv_r2_mean']:.3f}</div></div>
            <div class="algo-metric"><div class="algo-metric-label">MAE</div>
                <div class="algo-metric-value">{bm['mae']/100_000:.2f}L ₹</div></div>
            <div class="algo-metric"><div class="algo-metric-label">RMSE</div>
                <div class="algo-metric-value">{bm['rmse']/100_000:.2f}L ₹</div></div>
        </div>""", unsafe_allow_html=True)
    ...
    with b_content_col:
        ...
        <div class="format-card">
            ...
            <div class="format-row">
                ...
                CSV brut CarDekho — colonnes <code>name, year, km_driven, fuel, seller_type, transmission, owner</code>
            </div>
            <div class="format-row">
                <span class="format-tag">Format B</span>
                CSV nettoyé — colonnes <code>car_age</code> et <code>owner_encoded</code> déjà présentes
            </div>
            <div class="format-note">La colonne <code>selling_price</code> est ignorée si présente.</div>
        </div>""", unsafe_allow_html=True)

        exemple_csv = pd.DataFrame([
            {"name":"Maruti Swift VXI","year":2015,"km_driven":50000,"fuel":"Petrol","seller_type":"Individual","transmission":"Manual","owner":"First Owner"},
            {"name":"Hyundai i20 Asta","year":2018,"km_driven":30000,"fuel":"Diesel","seller_type":"Dealer","transmission":"Manual","owner":"Second Owner"},
            {"name":"Toyota Fortuner","year":2020,"km_driven":20000,"fuel":"Diesel","seller_type":"Dealer","transmission":"Automatic","owner":"First Owner"},
            {"name":"Honda City ZX","year":2016,"km_driven":65000,"fuel":"Petrol","seller_type":"Individual","transmission":"Manual","owner":"First Owner"},
            {"name":"Ford EcoSport Trend","year":2019,"km_driven":45000,"fuel":"Diesel","seller_type":"Dealer","transmission":"Manual","owner":"First Owner"},
        ])

        dl_col, up_col = st.columns([1, 1.6])
        with dl_col:
            st.download_button("Télécharger un exemple (5 véhicules)",
                data=exemple_csv.to_csv(index=False).encode("utf-8"),
                file_name="exemple_input_cardekho.csv", mime="text/csv", use_container_width=True)
        with up_col:
        ...
        if uploaded_file is not None:
            try:
                df_raw = (pd.read_excel(uploaded_file) if uploaded_file.name.endswith(".xlsx")
                          else pd.read_csv(uploaded_file))
                st.markdown(f'<p class="section-label">{len(df_raw)} véhicule(s) chargé(s)</p>', unsafe_allow_html=True)
                st.dataframe(df_raw.head(), use_container_width=True)

                with st.spinner("Prédictions en cours…"):
                    df_processed = preprocess_batch(df_raw)
                    preds = _predict_batch(selected_batch_model_name, selected_batch_model, df_processed)
                ...
                st.download_button("Télécharger les résultats (.csv)",
                    ...
                    file_name="predictions_cardekho.csv", mime="text/csv", use_container_width=True)
                st.success(f"{len(df_raw)} prédiction(s) générée(s) avec **{selected_batch_model_name}**.")
            except Exception as e:
                st.error(f"Erreur : {e}")
                st.exception(e)
...
with tab3:
    ...
    with t3_left:
        st.markdown('<p class="section-label">Résultats</p>', unsafe_allow_html=True)
        for mname, is_best in [("XGBoost",True),("Random Forest",False),("Régression Linéaire",False),("SVR",False)]:
            m = metrics[mname]
            badge = '<span class="summary-badge">MEILLEUR</span>' if is_best else ""
            st.markdown(f"""
            <div class="summary-card {'summary-card--best' if is_best else ''}">
                <div class="summary-card-header">
                    <span class="summary-card-name">{mname}</span>{badge}
                </div>
        ...
        <div class="conclusion-card">
            ...
            <strong>transmission</strong>, <strong>année</strong>, <strong>carburant (Diesel)</strong>.</p>
        </div>""", unsafe_allow_html=True)

    with t3_right:
        st.markdown('<p class="section-label">Comparaison des algorithmes</p>', unsafe_allow_html=True)
        st.markdown("4 algorithmes testés, optimisés via **GridSearchCV** et évalués par **validation croisée K-Fold (k=5)**.")

        rows = []
        for name, m in metrics.items():
            row = {"Modèle": name, "R² Test": m["r2"], "MAE (₹)": int(m["mae"]),
                   "RMSE (₹)": int(m["rmse"]), "CV R² Moy.": m["cv_r2_mean"], "CV R² Std": m["cv_r2_std"]}
            if "best_params" in m: row["Hyperparamètres"] = str(m["best_params"])
            rows.append(row)
        ...
        st.dataframe(df_cmp.style.apply(highlight_best,
            subset=["R² Test","MAE (₹)","RMSE (₹)","CV R² Moy."]), use_container_width=True)
        ...
        st.markdown("---")
        st.markdown('<p class="section-label">Importance des variables — XGBoost</p>', unsafe_allow_html=True)

        fi_data = {
            "transmission_Manual": 0.3163, "year": 0.2613, "fuel_Diesel": 0.2421,
            "brand_encoded": 0.0587, "seller_type_Individual": 0.0452, "km_driven": 0.0267,
            "seller_type_Trustmark Dealer": 0.0213, "owner_encoded": 0.0162,
            "fuel_Petrol": 0.0086, "fuel_LPG": 0.0034, "car_age": 0.0, "fuel_Electric": 0.0,
        }
        fi_max = max(fi_data.values()) or 1
        fi_rows = "".join(
            f'<div class="fi-row"><div class="fi-name">{var}</div>'
            f'<div class="fi-bar-wrap"><div class="fi-bar" style="width:{imp/fi_max*100:.1f}%"></div></div>'
            f'<div class="fi-pct">{imp*100:.1f}%</div></div>'
            for var, imp in fi_data.items()
        )
        st.markdown(f'<div style="margin-top:8px">{fi_rows}</div>', unsafe_allow_html=True)
